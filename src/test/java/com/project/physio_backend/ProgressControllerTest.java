...
public class ProgressControllerTest {
  ...
  void testCreateProgress() throws Exception {
    ...
    when(progressService.createProgress(eq(1L), eq(1L), any(Progress.class))).thenReturn(progress);

    mockMvc.perform(post("/api/progresses/1/1")
        .header("Authorization", "Bearer " + token)
        .contentType(MediaType.APPLICATION_JSON)
    ...
    verify(progressService, times(1)).createProgress(eq(1L), eq(1L), any(Progress.class));
  }
  ...
  @Test
  void testAddProgressPercentage() throws Exception {
    Progress progress = new Progress(LocalDateTime.now(), 50.0);
    progress.setProgressID(1L);

    when(progressService.addProgressPercentage(eq(1L), eq(25.0))).thenReturn(progress);

    mockMvc.perform(put("/api/progresses/1/percentage")
        .header("Authorization", "Bearer " + token)
        .contentType(MediaType.APPLICATION_JSON)
        .content(objectMapper.writeValueAsString(25.0)))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.percentag").value(50.0));

    verify(progressService, times(1)).addProgressPercentage(eq(1L), eq(25.0));
  }
}